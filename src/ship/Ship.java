...
public class Ship implements Runnable {
	...
	public void run() {
		try {
			...
			logger.error("Корабль " +this.getName() + " попытался заблокировать причал, не зарегистрировавшись ", e);
		}
	}
	...
	private boolean loadToPort(Berth berth) throws InterruptedException {
		...
		if (!result) {
			logger.debug("Недостаточно места на складе порта для выгрузки кораблем"
					+ name + " " + containersNumberToMove + " контейнеров, либо недостаточно контейнеров на складе корабля");
		} else {
	...
	private boolean loadFromPort(Berth berth) throws InterruptedException {
		...
		if (result) {
			...
			logger.debug("Недостаточно места  на корабле " + name
					+ " для погрузки " + containersNumberToMove + " контейнеров из порта, либо недостаточно контейнеров на складе порта");
		}
